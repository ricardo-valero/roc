...
, buildType ? "release", }:
let
  inherit (compile-deps) zigPkg llvmPkgs glibcPath libGccSPath darwinInputs;

  subPackagePath = if subPackage != null then "crates/${subPackage}" else null;
  mainBin =
    if subPackage == "language_server" then "roc_language_server" else "roc";
...
in rustPlatform.buildRustPackage {
  ...
  # required for zig
  XDG_CACHE_HOME =
    "xdg_cache"; # prevents zig AccessDenied error https://github.com/ziglang/zig/issues/6810
  ...
  nativeBuildInputs =
    builtins.attrValues { inherit (pkgs) cmake git pkg-config python3; }
    ++ [ llvmPkgs.dev llvmPkgs.lib zigPkg ];

  buildInputs = builtins.attrValues {
    inherit (pkgs)
      libffi libxml2 ncurses zlib cargo
      makeWrapper # necessary for postBuild wrapProgram
    ;
  } ++ darwinInputs;

  LLVM_SYS_180_PREFIX =
    lib.optionalString (llvmPkgs.dev != null) "${llvmPkgs.dev}";

  # cp: to copy str.zig,list.zig...
  # wrapProgram pkgs.stdenv.cc: to make ld available for compiler/build/src/link.rs
  postInstall = let
    binPath = lib.makeBinPath [ pkgs.stdenv.cc ];
